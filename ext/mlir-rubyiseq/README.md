...
This is an example of an out-of-tree [MLIR](https://mlir.llvm.org/) dialect along with a rubyiseq `opt`-like tool to operate on that dialect.
...
cmake --build . --target check-rubyiseq
```
...
cmake -G Ninja `$LLVM_SRC_DIR/llvm` \
    ...
    -DLLVM_EXTERNAL_PROJECTS=rubyiseq-dialect -DLLVM_EXTERNAL_RUBYISEQ_DIALECT_SOURCE_DIR=../
cmake --build . --target check-rubyiseq
```
